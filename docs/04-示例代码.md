...
Coroutine handleClient(GHandle handle, IOScheduler* scheduler) {
    ...
    while (true) {
        ...
        std::cout << "Received: " << std::string_view(buffer, bytes) << "\n";

        // 回显数据
        if (auto __await_result = co_await client.send(buffer, bytes); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
    }
...
Coroutine udpServer(IOScheduler* scheduler) {
    ...
    while (true) {
        ...
        // 回显数据
        if (auto __await_result = co_await socket.sendto(buffer, bytes, peer); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
    }
