...
Runtime runtime = RuntimeBuilder().ioSchedulerCount(16).computeSchedulerCount(4).build();  // 16个IO + 4个Compute

// CPU 密集型：更多 ComputeScheduler
Runtime runtime = RuntimeBuilder().ioSchedulerCount(4).computeSchedulerCount(16).build();  // 4个IO + 16个Compute

// 混合负载：平衡配置
Runtime runtime = RuntimeBuilder().ioSchedulerCount(8).computeSchedulerCount(8).build();   // 8个IO + 8个Compute

// 自动配置（推荐）
Runtime runtime = RuntimeBuilder().build();  // 2*CPU核数 IO + CPU核数 Compute
```
...
int main() {
    ...
    io->spawn([done]() -> Coroutine {
        // 任务逻辑
        if (auto __await_result = co_await doWork(); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
        done->store(true);
    }());
...
Coroutine goodIO() {
    ...
    if (openResult) {
        if (auto __await_result = co_await file.read(buffer, size, 0); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        } else {
            size_t bytes = __await_result.value();
            (void)bytes;
        }
        co_await file.close();
    }
...
Coroutine ioTask(Runtime* runtime) {
    ...
    // 继续 I/O 操作
    if (auto __await_result = co_await sendResult(result.value()); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
...
Coroutine task() {
    if (auto __await_result = co_await TimerAwaitable(std::chrono::seconds(1)); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
...
private:
    ...
    Coroutine worker() {
        while (true) {
            ...
            if (auto __await_result = co_await task(); !__await_result) {
                // 错误处理：记录日志、重试或提前返回
            }
        }
    }
...
public:
    Coroutine sendRequest(const Request& req, AsyncWaiter<Response>* waiter) {
        ...
        if (auto __await_result = co_await socket.send(data.data(), data.size()); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
...
public:
    ...
    Coroutine handleClient(TcpSocket client) {
        ...
        // 双向转发
        if (auto __await_result = co_await bidirectionalForward(client, backendSocket); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
...
Coroutine benchmarkTask(Benchmark* bench) {
    for (int i = 0; i < 10000; ++i) {
        if (auto __await_result = co_await doOperation(); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
        bench->recordOp();
    }
...
Coroutine measureLatency(LatencyTracker* tracker) {
    ...
    if (auto __await_result = co_await operation(); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
