...
Coroutine task() {
    int value = 42;
    if (auto __await_result = co_await someOperation(&value); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    } // value 可能在协程挂起时销毁
}
...
Coroutine task() {
    ...
    if (auto __await_result = co_await someOperation(value.get()); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
```
...
```cpp
Runtime runtime = RuntimeBuilder().ioSchedulerCount(8).computeSchedulerCount(4).build();  // 8个IO + 4个Compute
```

3. 使用零拷贝：

```cpp
if (auto __await_result = co_await socket.sendfile(fd, offset, size); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
