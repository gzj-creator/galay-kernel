...
| 场景 | 消息数 | 耗时 | 吞吐量 |
|------|--------|------|--------|
| 单生产者 | 1,000,000 | 36ms | 27,777,778 msg/s |
| 多生产者 (4线程) | 1,000,000 | 36ms | 27,777,778 msg/s |
| 批量接收 | 1,000,000 | 27ms | 37,037,037 msg/s |
| 跨调度器 | 1,000,000 | 35ms | 28,571,429 msg/s |

**持续负载测试 (5秒)：**
- 总发送: 83,678,146 消息
- 总接收: 83,678,146 消息
- 平均吞吐量: **15,872,182 msg/s**
...
| 场景 | 消息数 | 耗时 | 吞吐量 |
|------|--------|------|--------|
| 单生产者 | 1,000,000 | 111ms | 9,009,009 msg/s |
| 批量接收 | 1,000,000 | 85ms | 11,764,706 msg/s |

**延迟测试：**
- 消息数: 100,000
- 平均延迟: **5.96 μs**

**与 MpscChannel 对比（同调度器场景）：**

| Channel 类型 | 耗时 | 吞吐量 | 相对性能 |
|--------------|------|--------|----------|
| UnsafeChannel | 91ms | 10,989,011 msg/s | 1.32x |
| MpscChannel | 120ms | 8,333,333 msg/s | 1.00x |

> **结论**: 在同调度器场景下，UnsafeChannel 比 MpscChannel 快约 **1.32x**，适合对性能敏感的同调度器内协程通信。

### UnsafeChannel vs MpscChannel 特性对比
