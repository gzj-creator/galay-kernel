/**
 * @file T100-sequence_bidirectional_exclusive.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        std::perror("[T100] socketpair");
        return 1;
    }
    if (!setNonBlocking(fds[0]) || !setNonBlocking(fds[1])) {
        std::cerr << "[T100] failed to set non-blocking mode\n";
        close(fds[0]);
        close(fds[1]);
        return 1;
    }
    ...
    if (!first_in_position) {
        std::cerr << "[T100] bidirectional owner did not reach await point\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (!sendByteWithRetry(fds[1], kReadPayload)) {
        std::cerr << "[T100] failed to send wake payload\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
