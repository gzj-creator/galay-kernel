/**
 * @file T101-sequence_ordinary_conflict.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        std::perror("[T101] socketpair");
        return 1;
    }
    if (!setNonBlocking(fds[0]) || !setNonBlocking(fds[1])) {
        std::cerr << "[T101] failed to set non-blocking mode\n";
        close(fds[0]);
        close(fds[1]);
        return 1;
    }
    ...
    if (!sequence_in_position) {
        std::cerr << "[T101] read sequence did not reach await point\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (!sendByteWithRetry(fds[1], kPayload)) {
        std::cerr << "[T101] failed to send wake payload\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
