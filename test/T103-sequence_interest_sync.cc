/**
 * @file T103-sequence_interest_sync.cc
...
bool verifySingleOwnerInterestFollowsActiveTask() {
    ...
    if (read_mask != detail::sequenceSlotMask(IOController::READ) ||
        ...
        std::cerr << "[T103] read active task should produce read interest only\n";
        return false;
    }
    ...
    if (write_mask != detail::sequenceSlotMask(IOController::WRITE) ||
        ...
        std::cerr << "[T103] write active task should produce write interest only\n";
        return false;
    }
    ...
    if (empty_mask != 0 || controller.m_sequence_interest_mask != 0) {
        std::cerr << "[T103] invalid active task should clear controller interest\n";
        return false;
    }
...
int main() {
    ...
    std::cout << "T103-SequenceInterestSync PASS\n";
    return 0;
}
