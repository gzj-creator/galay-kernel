/**
 * @file T109-kqueue_sequence_persistent_registration_source_case.cc
...
int main() {
    ...
    if (content.empty()) {
        std::cerr << "[T109] failed to read source file: " << source << '\n';
        return 1;
    }

    if (!containsText(content, "append_change(IOController::READ, EV_ADD | EV_CLEAR);")) {
        std::cerr << "[T109] expected read-side sequence registration to use EV_CLEAR\n";
        return 1;
    }

    if (!containsText(content, "append_change(IOController::WRITE, EV_ADD | EV_CLEAR);")) {
        std::cerr << "[T109] expected write-side sequence registration to use EV_CLEAR\n";
        return 1;
    }
    ...
    if (add_accept.empty()) {
        std::cerr << "[T109] failed to isolate KqueueReactor::addAccept implementation\n";
        return 1;
    }
    if (!containsText(add_accept, "return kevent(m_kqueue_fd, &ev, 1, nullptr, 0, nullptr);")) {
        std::cerr << "[T109] expected addAccept to register immediately via kevent\n";
        return 1;
    }
    if (containsText(add_accept, "m_pending_changes.push_back(ev);")) {
        std::cerr << "[T109] expected addAccept to avoid delayed pending_changes registration\n";
        return 1;
    }
    ...
    if (containsText(content,
        ...
        std::cerr << "[T109] expected sequence event path to keep persistent armed mask\n";
        return 1;
    }

    std::cout << "T109-KqueueSequencePersistentRegistrationSourceCase PASS\n";
    return 0;
}
