/**
 * @file T113-ioscheduler_inject_ring_fallback.cc
...
bool runScenario() {
    ...
    if (!single_item_worker.local_ring.push_back(makeTaskRef())) {
        std::cerr << "[T113] failed to enqueue single-item ring probe\n";
        return false;
    }
    TaskRef single_popped;
    if (!single_item_worker.local_ring.pop_back(single_popped)) {
        std::cerr << "[T113] failed to pop single-item ring probe\n";
        return false;
    }
    if (!single_item_worker.local_ring.empty()) {
        std::cerr << "[T113] ring should be empty after single-item pop_back\n";
        return false;
    }
    ...
    for (size_t i = 0; i < fill_count; ++i) {
        if (!worker.local_ring.push_back(makeTaskRef())) {
            std::cerr << "[T113] failed to fill ring (index=" << i << ")\n";
            return false;
        }
    }
    ...
    if (worker.injected_outstanding.load(std::memory_order_acquire) != total_injected) {
        std::cerr << "[T113] expected injected_outstanding == " << total_injected
    ...
    if (first_drain != kRemainingCapacity) {
        std::cerr << "[T113] first drain should move " << kRemainingCapacity
                  << " tasks, moved=" << first_drain << "\n";
        return false;
    }
    if (!worker.hasPendingInjected()) {
        std::cerr << "[T113] expected pending injected tasks after first drain\n";
        return false;
    }
    ...
    for (size_t i = 0; i < kRemainingCapacity; ++i) {
        ...
        if (!worker.local_ring.pop_back(popped)) {
            std::cerr << "[T113] failed to free ring slot " << i << "\n";
            return false;
        }
    }
    ...
    if (second_drain != expected_second) {
        std::cerr << "[T113] second drain expected " << expected_second
                  << " tasks, moved=" << second_drain << "\n";
        return false;
    }
    if (worker.hasPendingInjected()) {
        std::cerr << "[T113] no pending injected tasks expected after second drain\n";
        return false;
    }

    if (worker.injected_outstanding.load(std::memory_order_acquire) != 0) {
        std::cerr << "[T113] injected_outstanding should be 0 after draining\n";
        return false;
    }
...
int main() {
    ...
    std::cout << "T113-ioscheduler_inject_ring_fallback PASS\n";
    return 0;
}
