/**
 * @file T116-chase_lev_ring_steal_pop_stress.cc
...
bool runStressScenario() {
    ...
    if (consumed != kRoundCount) {
        std::cerr << "[T116] consumed count mismatch, expected=" << kRoundCount
                  << ", actual=" << consumed << "\n";
        return false;
    }
    ...
    if (duplicate_count != 0) {
        std::cerr << "[T116] duplicate consumption detected, duplicates="
                  << duplicate_count << "\n";
        return false;
    }
...
int main() {
    ...
    std::cout << "T116-ChaseLevRingStealPopStress PASS\n";
    return 0;
}
