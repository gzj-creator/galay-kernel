/**
 * @file T118-ioscheduler_local_deferred_fifo.cc
...
bool runScenario() {
    ...
    if (drained != 3) {
        std::cerr << "[T118] expected deferred staging to drain 3 tasks, actual="
                  << drained << "\n";
        return false;
    }
    ...
    for (uint64_t expected = 0; expected < 3; ++expected) {
        ...
        if (!worker.popNext(next)) {
            std::cerr << "[T118] expected deferred task " << expected << " to be available\n";
            return false;
        }
        const uint64_t actual = taggedTaskId(next);
        if (actual != expected) {
            std::cerr << "[T118] deferred FIFO violated, expected=" << expected
                      << ", actual=" << actual << "\n";
            return false;
        }
    }
    ...
    if (worker.hasLocalWork() || worker.hasPendingInjected()) {
        std::cerr << "[T118] worker should be empty after draining deferred FIFO\n";
        return false;
    }
...
int main() {
    ...
    std::cout << "T118-IOSchedulerLocalDeferredFIFO PASS\n";
    return 0;
}
