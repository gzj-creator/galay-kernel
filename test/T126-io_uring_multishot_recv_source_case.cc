/**
 * @file T126-io_uring_multishot_recv_source_case.cc
...
int main() {
    ...
    if (iocontroller_text.empty() || iouring_h_text.empty() || iouring_cc_text.empty()) {
        std::cerr << "[T126] failed to read source files\n";
        return 1;
    }

    if (!containsText(iocontroller_text, "m_ready_recvs")) {
        std::cerr << "[T126] expected IOController to keep queued ready recv payloads\n";
        return 1;
    }
    if (!containsText(iocontroller_text, "m_recv_multishot_armed")) {
        std::cerr << "[T126] expected IOController to track multishot recv armed state\n";
        return 1;
    }
    if (!containsText(iocontroller_text, "m_recv_multishot_token")) {
        std::cerr << "[T126] expected IOController to track active multishot recv token\n";
        return 1;
    }

    if (!containsText(iouring_h_text, "processRecvCompletion")) {
        std::cerr << "[T126] expected IOUringReactor to define processRecvCompletion helper\n";
        return 1;
    }
    if (!containsText(iouring_cc_text, "io_uring_setup_buf_ring(")) {
        std::cerr << "[T126] expected IOUringReactor to allocate a provided buffer ring\n";
        return 1;
    }
    if (!containsText(iouring_cc_text, "io_uring_prep_recv_multishot(")) {
        std::cerr << "[T126] expected IOUringReactor recv path to use multishot recv submission\n";
        return 1;
    }
    if (!containsText(iouring_cc_text, "IOSQE_BUFFER_SELECT")) {
        std::cerr << "[T126] expected IOUringReactor recv SQE to enable IOSQE_BUFFER_SELECT\n";
        return 1;
    }
    if (!containsText(iouring_cc_text, "IORING_CQE_F_BUFFER")) {
        std::cerr << "[T126] expected recv completion path to inspect IORING_CQE_F_BUFFER\n";
        return 1;
    }
    if (!containsText(iouring_cc_text, "io_uring_buf_ring_add(") ||
        !containsText(iouring_cc_text, "io_uring_buf_ring_advance(")) {
        std::cerr << "[T126] expected IOUringReactor to return consumed buffers back to buf ring\n";
        return 1;
    }

    std::cout << "T126-IOUringMultishotRecvSourceCase PASS\n";
    return 0;
}
