...
bool runSameThreadWakeScenario() {
    ...
    if (!waiter_done || !old_done) {
        std::cerr << "[T32] same-thread wake scenario timed out\n";
        return false;
    }
    ...
    if (old_pos < 0 || resumed_pos < 0) {
        std::cerr << "[T32] same-thread wake trace incomplete\n";
        return false;
    }

    if (resumed_pos > old_pos) {
        std::cerr << "[T32] expected same-thread wake to run resumed task before older queued work"
...
bool runCrossThreadWakeScenario() {
    ...
    if (!done) {
        std::cerr << "[T32] cross-thread wake scenario timed out\n";
        return false;
    }
...
bool runFairnessScenario() {
    ...
    if (!old_done || !hot_done) {
        std::cerr << "[T32] fairness scenario timed out\n";
        return false;
    }

    if (state.old_after_hot.load(std::memory_order_acquire)) {
        std::cerr << "[T32] expected older queued work to run before hot same-thread reschedule chain completed\n";
        return false;
    }
...
int main() {
    ...
    std::cout << "T32-IOSchedulerLocalFirst PASS\n";
    return 0;
}
