...
int main() {
    ...
    for (int i = 0; i < kMessageCount; ++i) {
        if (!channel.send(i)) {
            std::cerr << "[T58] failed to preload message " << i << "\n";
            return 1;
        }
    }
    ...
    if (!first || *first != 0) {
        std::cerr << "[T58] expected first value 0\n";
        return 1;
    }

    const size_t prefetched = MpscChannelTestAccess::prefetchedCount(channel);
    if (prefetched == 0) {
        std::cerr << "[T58] expected single recv prefetch buffer to be populated\n";
        return 1;
    }
    if (prefetched > kPrefetchLimit) {
        std::cerr << "[T58] prefetch buffer exceeded configured limit\n";
        return 1;
    }

    if (channel.size() != static_cast<size_t>(kMessageCount - 1)) {
        std::cerr << "[T58] expected size to stay at " << (kMessageCount - 1)
                  << ", got " << channel.size() << "\n";
        return 1;
    }
    ...
    if (!firstBatch || firstBatch->size() != kDefaultBatchSize) {
        std::cerr << "[T58] expected no-arg tryRecvBatch to use configured batch size\n";
        return 1;
    }

    for (size_t i = 0; i < kDefaultBatchSize; ++i) {
        if ((*firstBatch)[i] != static_cast<int>(i + 1)) {
            std::cerr << "[T58] ordering mismatch at index " << (i - 1) << "\n";
            return 1;
        }
    }
    ...
    if (!rest || rest->size() != static_cast<size_t>(kMessageCount - 1 - kDefaultBatchSize)) {
        std::cerr << "[T58] expected second batch to drain remaining messages\n";
        return 1;
    }
    ...
    for (size_t i = 0; i < rest->size(); ++i) {
        ...
        if ((*rest)[i] != expected) {
            std::cerr << "[T58] trailing ordering mismatch at index " << i << "\n";
            return 1;
        }
    }

    if (MpscChannelTestAccess::prefetchedCount(channel) != 0) {
        std::cerr << "[T58] expected prefetch buffer to be empty after full drain\n";
        return 1;
    }

    if (!channel.empty()) {
        std::cerr << "[T58] expected channel to be empty after draining all messages\n";
        return 1;
    }

    std::cout << "T58-MpscSingleRecvPrefetch PASS\n";
    return 0;
}
