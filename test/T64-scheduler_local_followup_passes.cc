...
bool verifyLocalFollowupPassesDrainLocalBacklog() {
    ...
    if (summary.ran != kTaskCount || resumed != kTaskCount) {
        std::cerr << "[T64] local follow-up passes should drain full local backlog, ran="
                  << summary.ran << ", resumed=" << resumed << "\n";
        return false;
    }

    if (summary.drainedRemote != 0) {
        std::cerr << "[T64] local backlog should not report remote drains, drained="
                  << summary.drainedRemote << "\n";
        return false;
    }

    if (summary.passes <= 1) {
        std::cerr << "[T64] local backlog should require multiple passes when over budget, passes="
                  << summary.passes << "\n";
        return false;
    }

    if (worker.hasLocalWork()) {
        std::cerr << "[T64] local backlog should be empty after follow-up passes\n";
        return false;
    }
...
bool verifyRemoteDrainDoesNotNeedFollowupPasses() {
    ...
    if (summary.ran != kTaskCount || resumed != kTaskCount) {
        std::cerr << "[T64] injected backlog should still complete, ran="
                  << summary.ran << ", resumed=" << resumed << "\n";
        return false;
    }

    if (summary.drainedRemote != kTaskCount || remote_drained != kTaskCount) {
        std::cerr << "[T64] injected backlog should report one remote drain of full backlog, summary="
                  << summary.drainedRemote << ", callback=" << remote_drained << "\n";
        return false;
    }

    if (summary.passes != 1) {
        std::cerr << "[T64] injected burst fast-path should complete in one pass, passes="
                  << summary.passes << "\n";
        return false;
    }
...
int main() {
    ...
    std::cout << "T64-SchedulerLocalFollowupPasses PASS\n";
    return 0;
}
