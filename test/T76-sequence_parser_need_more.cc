/**
 * @file T76-sequence_parser_need_more.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        std::cerr << "[T76] socketpair failed: " << std::strerror(errno) << "\n";
        return 1;
    }
    ...
    if (::send(fds[1], frame.data(), 6, 0) != 6) {
        std::cerr << "[T76] failed to send partial frame\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (state.done.load(std::memory_order_acquire)) {
        std::cerr << "[T76] coroutine resumed before full frame arrived\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (::send(fds[1], frame.data() + 6, frame.size() - 6, 0) != static_cast<ssize_t>(frame.size() - 6)) {
        std::cerr << "[T76] failed to send remaining frame\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
