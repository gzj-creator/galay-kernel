/**
 * @file T77-sequence_parser_coalesced_frames.cc
...
int main() {
    ...
    if (::send(fds[1], payload.data(), payload.size(), 0) != static_cast<ssize_t>(payload.size())) {
        std::cerr << "[T77] failed to send coalesced frames\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
