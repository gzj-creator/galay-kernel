/**
 * @file T84-epoll_aio_eventfd_rearm.cc
...
int main()
{
    ...
    if (!scheduleTask(scheduler, aioWriteThenRead(&done, &passed))) {
        scheduler.stop();
        std::cerr << "T84 scheduleTask failed\n";
        return 1;
    }
    ...
    if (!done.load(std::memory_order_acquire)) {
        std::cerr << "T84 timeout waiting for second AIO completion\n";
        return 1;
    }
    if (!passed.load(std::memory_order_acquire)) {
        std::cerr << "T84 read-after-write verification failed\n";
        return 1;
    }

    std::cout << "T84-EpollAioEventfdRearm PASS\n";
    return 0;
}
...
int main()
{
    std::cout << "T84-EpollAioEventfdRearm SKIP (non-epoll backend)\n";
    return 0;
}
