/**
 * @file T88-state_machine_fail_action.cc
...
int main() {
    ...
    if (!completed) {
        std::cerr << "[T88] state machine fail action timed out\n";
        return 1;
    }
    if (!state.success.load(std::memory_order_acquire)) {
        std::cerr << "[T88] state machine fail action mismatch\n";
        return 1;
    }

    std::cout << "T88-StateMachineFailAction PASS\n";
    return 0;
}
