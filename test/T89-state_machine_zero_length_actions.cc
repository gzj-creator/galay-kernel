/**
 * @file T89-state_machine_zero_length_actions.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        std::cerr << "[T89] socketpair failed\n";
        return 1;
    }
    ...
    if (!completed) {
        std::cerr << "[T89] zero-length state machine timed out\n";
        return 1;
    }
    if (!state.success.load(std::memory_order_acquire)) {
        std::cerr << "[T89] zero-length state machine result mismatch\n";
        return 1;
    }

    std::cout << "T89-StateMachineZeroLengthActions PASS\n";
    return 0;
}
