/**
 * @file T90-awaitable_builder_connect_bridge.cc
...
int main() {
    ...
    if (listen_fd < 0) {
        std::cerr << "[T90] createListenSocket failed: " << std::strerror(errno) << "\n";
        return 1;
    }
    ...
    if (client_fd < 0) {
        close(listen_fd);
        accept_thread.join();
        std::cerr << "[T90] client socket failed: " << std::strerror(errno) << "\n";
        return 1;
    }
    ...
    if (!completed) {
        std::cerr << "[T90] builder connect timed out\n";
        return 1;
    }
    if (!state.success.load(std::memory_order_acquire)) {
        std::cerr << "[T90] builder connect result mismatch\n";
        return 1;
    }
    if (!server_accepted) {
        std::cerr << "[T90] server did not observe the connection\n";
        return 1;
    }

    std::cout << "T90-AwaitableBuilderConnectBridge PASS\n";
    return 0;
}
