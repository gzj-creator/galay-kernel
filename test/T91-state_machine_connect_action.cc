/**
 * @file T91-state_machine_connect_action.cc
...
int main() {
    ...
    if (listen_fd < 0) {
        std::cerr << "[T91] createListenSocket failed: " << std::strerror(errno) << "\n";
        return 1;
    }
    ...
    if (client_fd < 0) {
        close(listen_fd);
        accept_thread.join();
        std::cerr << "[T91] client socket failed: " << std::strerror(errno) << "\n";
        return 1;
    }
    ...
    if (!completed) {
        std::cerr << "[T91] state machine connect timed out\n";
        return 1;
    }
    if (!state.success.load(std::memory_order_acquire)) {
        std::cerr << "[T91] state machine connect result mismatch\n";
        return 1;
    }
    if (!server_accepted) {
        std::cerr << "[T91] server did not observe the connection\n";
        return 1;
    }

    std::cout << "T91-StateMachineConnectAction PASS\n";
    return 0;
}
