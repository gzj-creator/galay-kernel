/**
 * @file T92-awaitable_builder_queue_rejected.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        std::cerr << "[T92] socketpair failed: " << std::strerror(errno) << "\n";
        return 1;
    }
    ...
    if (!completed) {
        std::cerr << "[T92] builder queue rejection timed out\n";
        return 1;
    }
    if (!state.success.load(std::memory_order_acquire)) {
        std::cerr << "[T92] builder queue rejection mismatch\n";
        return 1;
    }

    std::cout << "T92-AwaitableBuilderQueueRejected PASS\n";
    return 0;
}
