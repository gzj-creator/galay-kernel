/**
 * @file T94-awaitable_builder_iovec_roundtrip.cc
...
int main() {
    ...
    if (!completed) {
        std::cerr << "[T94] builder iovec roundtrip timed out\n";
        return 1;
    }
    if (!state.writer_ok.load(std::memory_order_acquire) ||
        !state.reader_ok.load(std::memory_order_acquire)) {
        std::cerr << "[T94] builder iovec roundtrip mismatch\n";
        return 1;
    }

    std::cout << "T94-AwaitableBuilderIovecRoundtrip PASS\n";
    return 0;
}
