/**
 * @file T95-awaitable_builder_iovec_parse_bridge.cc
...
int main() {
    ...
    if (!first_fragment_observed) {
        ...
        peer.join();
        close(fds[1]);
        std::cerr << "[T95] first fragment was not observed\n";
        return 1;
    }
    ...
    if (!first_read_seen || !need_more_seen || state.done.load(std::memory_order_acquire)) {
        ...
        peer.join();
        close(fds[1]);
        std::cerr << "[T95] builder iovec parse bridge did not suspend on the first fragment\n";
        return 1;
    }
    state.allow_second_fragment.store(true, std::memory_order_release);
    ...
    if (!completed) {
        std::cerr << "[T95] builder iovec parse bridge timed out\n";
        return 1;
    }
    if (!state.success.load(std::memory_order_acquire) ||
        !state.ack_ok.load(std::memory_order_acquire)) {
        std::cerr << "[T95] builder iovec parse bridge mismatch\n";
        return 1;
    }

    std::cout << "T95-AwaitableBuilderIovecParseBridge PASS\n";
    return 0;
}
