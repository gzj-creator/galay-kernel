/**
 * @file T96-state_machine_timeout.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, direct_fds) != 0) {
        std::perror("[T96] direct socketpair");
        return 1;
    }
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, builder_fds) != 0) {
        std::perror("[T96] builder socketpair");
        close(direct_fds[0]);
        close(direct_fds[1]);
        return 1;
    }
    if (!setNonBlocking(direct_fds[0]) || !setNonBlocking(direct_fds[1]) ||
        ...
        std::cerr << "[T96] failed to set non-blocking mode\n";
        close(direct_fds[0]);
        close(direct_fds[1]);
        close(builder_fds[0]);
        close(builder_fds[1]);
    ...
    if (!direct_completed) {
        std::cerr << "[T96] direct state-machine timeout task did not complete\n";
        return 1;
    }
    if (!builder_completed) {
        std::cerr << "[T96] builder state-machine timeout task did not complete\n";
        return 1;
    }
    if (state.direct_error.load(std::memory_order_acquire) != static_cast<int>(kTimeout)) {
        std::cerr << "[T96] direct state-machine timeout error mismatch: "
    ...
    if (state.builder_error.load(std::memory_order_acquire) != static_cast<int>(kTimeout)) {
        std::cerr << "[T96] builder state-machine timeout error mismatch: "
                  << state.builder_error.load(std::memory_order_acquire) << "\n";
        return 1;
    }

    std::cout << "T96-StateMachineTimeout PASS\n";
    return 0;
}
