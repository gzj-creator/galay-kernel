/**
 * @file T97-state_machine_await_context.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, direct_fds) != 0) {
        std::perror("[T97] direct socketpair");
        return 1;
    }
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, builder_fds) != 0) {
        std::perror("[T97] builder socketpair");
        close(direct_fds[0]);
        close(direct_fds[1]);
        return 1;
    }
    ...
    if (!direct_completed || !state.direct_success.load(std::memory_order_acquire)) {
        std::cerr << "[T97] direct state-machine await context hook failed\n";
        return 1;
    }
    if (!builder_completed || !state.builder_success.load(std::memory_order_acquire)) {
        std::cerr << "[T97] builder flow await context hook failed\n";
        return 1;
    }

    std::cout << "T97-StateMachineAwaitContext PASS\n";
    return 0;
}
