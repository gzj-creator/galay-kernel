/**
 * @file T98-sequence_owner_conflict.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        std::perror("[T98] socketpair");
        return 1;
    }
    if (!setNonBlocking(fds[0]) || !setNonBlocking(fds[1])) {
        std::cerr << "[T98] failed to set non-blocking mode\n";
        close(fds[0]);
        close(fds[1]);
        return 1;
    }
    ...
    if (!first_in_position) {
        std::cerr << "[T98] first read sequence did not reach await point\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (!sendByteWithRetry(fds[1], kPayload)) {
        std::cerr << "[T98] failed to send wake payload\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (!first_completed) {
        std::cerr << "[T98] first sequence owner did not complete\n";
        return 1;
    }
    if (state.first_value.load(std::memory_order_acquire) != 1) {
        std::cerr << "[T98] first sequence owner read unexpected byte count: "
                  << state.first_value.load(std::memory_order_acquire) << "\n";
        return 1;
    }
    if (!second_failed_fast) {
        std::cerr << "[T98] second read sequence was not rejected immediately\n";
        return 1;
    }
    if (state.second_error.load(std::memory_order_acquire) != static_cast<int>(kNotReady)) {
        std::cerr << "[T98] second sequence owner error mismatch: "
                  << state.second_error.load(std::memory_order_acquire) << "\n";
        return 1;
    }

    std::cout << "T98-SequenceOwnerConflict PASS\n";
    return 0;
}
