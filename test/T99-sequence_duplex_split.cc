/**
 * @file T99-sequence_duplex_split.cc
...
int main() {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        std::perror("[T99] socketpair");
        return 1;
    }
    if (!setNonBlocking(fds[0]) || !setNonBlocking(fds[1])) {
        std::cerr << "[T99] failed to set non-blocking mode\n";
        close(fds[0]);
        close(fds[1]);
        return 1;
    }
    if (!fillSendBuffer(fds[0])) {
        std::cerr << "[T99] failed to pre-fill socket send buffer\n";
        close(fds[0]);
        close(fds[1]);
        return 1;
    }
    ...
    if (!suspend_barrier_ready) {
        std::cerr << "[T99] read/write inner await_suspend did not both run\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (!sendByteWithRetry(fds[1], kReadPayload)) {
        std::cerr << "[T99] failed to inject read payload\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
    ...
    if (!drainPeerUntilWriteCompletes(&state, fds[1])) {
        std::cerr << "[T99] failed to drain peer buffer until write completion\n";
        scheduler.stop();
        close(fds[0]);
        close(fds[1]);
        return 1;
